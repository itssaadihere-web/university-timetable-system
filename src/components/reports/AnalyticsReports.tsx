  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500">Physical Classrooms</span>
            <DoorOpen className="w-4 h-4 text-teal-600" />
          </div>
          <p className="text-2xl font-extrabold text-slate-900 mt-2">{realRooms.length}</p>
          <p className="text-[11px] text-slate-400 mt-0.5">Avg Utilization: {avgUtilization}%</p>
        </div>

        {/* Priority KPI: Missing Room Details */}
        <div className={`rounded-2xl p-4 border shadow-2xs transition-all ${
          ...
          <div className="flex items-center justify-between">
            <span className={`text-xs font-bold ${unassignedSessions.length > 0 ? 'text-rose-900' : 'text-slate-500'}`}>
              Pending Room Allocations
            </span>
            <AlertCircle className={`w-4 h-4 ${unassignedSessions.length > 0 ? 'text-rose-600 animate-pulse' : 'text-slate-400'}`} />
          </div>
          <p className={`text-2xl font-extrabold mt-2 ${unassignedSessions.length > 0 ? 'text-rose-700' : 'text-slate-900'}`}>
            {unassignedSessions.length}
          </p>
          <p className={`text-[11px] font-semibold mt-0.5 ${unassignedSessions.length > 0 ? 'text-rose-800' : 'text-emerald-600'}`}>
            {unassignedSessions.length > 0 ? '⚠️ High priority detail missing' : '✓ All classes assigned'}
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs p-4 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {activeTab === 'rooms' && (
              <div className="flex items-center gap-2">
                <span className="text-xs text-slate-500 font-semibold">Specialty Filter:</span>
                <select
                  value={roomTagFilter}
                  onChange={(e) => setRoomTagFilter(e.target.value)}
                  className="px-2.5 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-shu-700/20"
                >
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={activeTab === 'faculty' ? 'Search teacher or department...' : 'Search room name or building...'}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-8 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-800 placeholder-slate-400 w-52 sm:w-64 focus:outline-none focus:ring-2 focus:ring-shu-700/20"
              />
              {searchQuery && (
                <button
                    ...
                    {canEdit && (
                      <td className="py-3.5 px-4 text-right">
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors border border-indigo-200 cursor-pointer"
                        >
      ...
      {activeTab === 'rooms' && (
        <div className="space-y-5">
          {/* Top Priority Missing Room Allocations Panel */}
          {unassignedSessions.length > 0 && (
            <div className="bg-gradient-to-br from-amber-50 via-rose-50 to-orange-50 border-2 border-amber-300 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-amber-200/80 pb-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-200 text-amber-950">
                        {unassignedSessions.length} Classes Missing Venue
                      </span>
                    </div>
                    <h3 className="text-base sm:text-lg font-extrabold text-slate-900 mt-1">
                      Class Sessions Missing Physical Classroom Allocation
                    </h3>
                    <p className="text-xs text-slate-600 mt-0.5 max-w-2xl">
                      "Room Not Assigned" is treated as a critical missing schedule detail. The Program Coordinator should assign physical classrooms (lecture halls, labs, or seminar rooms) to all scheduled classes on top priority.
                    </p>
                  </div>
                </div>
                ...
                {onOpenRoomAllocation && (
                  <button
                    onClick={onOpenRoomAllocation}
                    className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-extrabold bg-shu-700 hover:bg-shu-800 active:bg-shu-900 text-white rounded-xl shadow-sm transition-all shrink-0 cursor-pointer self-start sm:self-auto"
                  >
                    <DoorOpen className="w-4 h-4" />
                    <span>Launch Room Allocator ({unassignedSessions.length})</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                )}
              </div>

              {/* Table of Unassigned Sessions */}
              <div className="bg-white/90 backdrop-blur rounded-2xl border border-amber-200 overflow-hidden shadow-2xs">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-amber-100/70 border-b border-amber-200 text-amber-950 font-bold uppercase tracking-wider text-[10px]">
                      <tr>
                        <th className="py-2.5 px-3.5">Course Code & Name</th>
                        <th className="py-2.5 px-3.5">Instructor</th>
                        <th className="py-2.5 px-3.5">Batch / Section</th>
                        <th className="py-2.5 px-3.5">Scheduled Day & Time</th>
                    ...
                    <tbody className="divide-y divide-amber-100/80 font-medium text-slate-800">
                      {unassignedSessions.map((session) => {
                        ...
                        return (
                          <tr key={session.id} className="hover:bg-amber-50/60 transition-colors">
                            <td className="py-2.5 px-3.5 font-bold text-slate-900">
                              <span className="font-mono text-indigo-700 font-extrabold mr-1.5">
                                {crs?.code || 'CRS-000'}
                              </span>
                              <span>{crs?.name || 'Class Session'}</span>
                            </td>
                            <td className="py-2.5 px-3.5 text-slate-700">{fac?.name || 'Instructor'}</td>
                            ...
                            <td className="py-2.5 px-3.5">
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-amber-200 text-amber-950 border border-amber-300">
                                <AlertTriangle className="w-3 h-3 text-amber-700" /> Room Missing (Pending)
                              </span>
                            </td>
                            {canEdit && (
                              <td className="py-2.5 px-3.5 text-right">
                                <button
                                  onClick={() => {
                                    ...
                                    } else if (onOpenRoomAllocation) {
                                      onOpenRoomAllocation();
                                    }
                                  }}
                                  className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold text-white bg-shu-700 hover:bg-shu-800 rounded-lg transition-colors shadow-2xs cursor-pointer"
                                >
          ...
          {/* Genuine Physical Rooms Utilization Table */}
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
            <div className="p-4 bg-slate-50/80 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h4 className="font-extrabold text-slate-900 text-sm">
                  Physical Classroom Inventory & Weekly Utilization
                </h4>
                <p className="text-xs text-slate-500">
                  Showing {filteredRooms.length} university classrooms & laboratories.
                </p>
              </div>
              <div className="text-xs text-slate-500 font-mono">
                Utilization baseline: 32.5h / week
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                      ...
                      {canEdit && (
                        <td className="py-3.5 px-4 text-right">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold text-teal-700 bg-teal-50 hover:bg-teal-100 rounded-lg transition-colors border border-teal-200 cursor-pointer"
                          >
                            <Edit3 className="w-3 h-3" />
